...
- Below is the real answer I got from the tool `cyberchef`.   
    - I'm sorry for my mistake. The decode from `Base64` is `Jverex Av Jfrcre Ez Brre` but I paste at the tool `cyberchef` with more letter r. So the output is `Jverex Av Jfrcre Ez Brrre` is wrong. That's why got an error.
